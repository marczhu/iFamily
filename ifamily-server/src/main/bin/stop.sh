...
PIDPROC=`ps -ef | grep 'ifamily-server/conf' | grep $INSTANCE_NAME_SYMBOL | grep -v 'grep'| awk '{print $2}'`

if [ -z "$PIDPROC" ];then
 echo "ifamily-server($INSTANCE_NAME_SYMBOL) is not running"
 exit 0
fi
...
do
if kill -9 $PID
   then echo "process ifamily-server(Pid:$PID symbol:$INSTANCE_NAME_SYMBOL) was force stopped at " `date`
fi
done
echo stop finished.
